...
assert.doesNotMatch(html, /载入示例|清空|type="file"|showDirectoryPicker|选择任务目录/);
...
node('snapshot').textContent=JSON.stringify(snapshot);
node('search').value='';
let reloads=0, fetches=0;
const listeners={};
const scope={
  document:{getElementById:node,hidden:false,addEventListener(type,fn){listeners[type]=fn;}},
  window:{},
...
assert.match(node('status').textContent,/当前项目/);
...
(async()=>{
  ...
  vm.runInContext('live=true',scope);
