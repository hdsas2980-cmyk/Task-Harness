...
- 默认先用 `spawn_agent` 拆分独立的探索、测试、分诊、总结和校验工作；只有需要长期上下文、用户可见跟进、隔离写入或独立评审时才编排会话。
...
启用 task-harness 后，独立读密集卡优先 `spawn_agent`；用户要侧边栏跟进、任务需要长期独立上下文、写密集卡需要 worktree 隔离，或任务属于独立评审时，使用 `create_thread`。用户启用本技能即授权按此规则委托，不必另等「开新会话」指令。
...
|---|---|
| 用户要在侧边栏看见、要跟、要长期线 | 探索、测试、分诊、总结、日志分析、依赖图、格式校验、快速查询 |
| 独立评审 / 审计 | 当前会话不依赖结果即可继续的并行小卡 |
| 写密集任务需要独立 worktree 或独立上下文 | 无共享状态的读密集工作 |
...
**场景**: 用户需要长期可见会话、隔离写入或独立评审。主线派发后继续推进，各会话完成后主动通知主线。
...
**关键**: 不要用 `wait_threads` 做轮询或普通跟进；让子会话完成后通过 `send_message_to_thread` 或用户界面的"由 ChatGPT 从另一项任务发送"通知主线。只有下一步必须收齐多个最终结果时，才用一次 `wait_threads` 结果屏障。
...
**关键**: 子代理不承担独立评审，不直接把任务置为 `passed`。主线必须核对其摘要、实际文件和可重放证据。子代理结果是硬门槛时，用 `wait_agent` 收取指定代理；不要用 `wait_threads` 代替子代理结果收取。
...
- 用 `wait_threads` 轮询或普通跟进子会话；应使用主动回报，只有收集多个最终结果时才使用一次性屏障。
