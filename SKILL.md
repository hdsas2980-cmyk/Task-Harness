...
- `dashboard/`：独立看板（启动探测当前目录；载入 = 另选项目任务目录）与 Tauri 桌面壳。
...
- 「载入任务」选择项目任务目录（`.harness` 或项目根）；「刷新任务」重读该目录。桌面壳 `TaskHarness.exe` 启动先探测当前工作目录 / exe 目录，点「载入任务」才改选。技能初始化仍可先载入当前 HTTP 项目，随后也能改选目录。
