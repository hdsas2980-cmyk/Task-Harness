...
分派规则：
- 每个子 Agent 严格「只领一个任务」：置 active → 只读该任务及触及代码 → 实现 → 跑 verify → 追加 `.harness/evidence.jsonl` → 置 evidence_ready → 调 `gstack/review` 独立评审 → 按 `HARNESS_REVIEW:` 一行结论更新 status 并追加 `.harness/reviews.jsonl` → 追加一句 `.harness/progress.txt`。
