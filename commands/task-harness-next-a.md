---
name: task-harness-next-a
description: task-harness v3.2 (TRAE) 推进 · A 单步（默认，不进 loop）。输入 /task-harness-next-a 触发。
---

按 task-harness v3.2 单步推进一轮。只推进一个任务，禁止回读全量清单与旧证据，保持主会话上下文恒定。

1. 读 `.harness/progress.txt` 最后一条（只读最后一条，不回溯全文）。
2. 运行 `.harness/init.ps1`（Windows PowerShell 默认）或 `.harness/init.sh`（bash）→ 看紧凑状态：进度计数 + 下一个 eligible 任务。
